...
fn main() {
    ...
    match build_theme_set() {
        ...
        Err(e) => {
            ...
            eprintln!("Creating empty theme set as fallback");
...
fn generate_themes_module(theme_set: &syntect::highlighting::ThemeSet, out_dir: &Path) {
    ...
    // Generate the THEMES map
    content.push_str("pub fn load_theme(name: &str) -> Option<Theme> {\n");
    content.push_str("    let bytes: &[u8] = match name {\n");

    for name in &theme_names {
        let safe_filename = name.replace(' ', "_").replace("(", "").replace(")", "");
        content.push_str(&format!(
            "        {:?} => include_bytes!(concat!(env!(\"OUT_DIR\"), \"/themes/{}.bin\")),\n",
            name, safe_filename
        ));
    }
    ...
    content.push_str("    };\n");
    content.push_str("    Some(syntect::dumps::from_binary(bytes))\n");
    content.push_str("}\n\n");
