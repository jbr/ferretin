...
fn test_mode_transitions_via_state() {
    ...
    assert!(matches!(state.ui_mode, UiMode::Input(InputMode::GoTo { .. })));
    ...
    assert!(
        matches!(
            state.ui_mode,
            UiMode::Input(InputMode::Search { all_crates: false, .. })
        )
    );
}
...
fn test_rendering_to_test_backend() {
    ...
    let buffer_str = buffer.content().iter().map(|cell| cell.symbol()).collect::<String>();
    assert!(
